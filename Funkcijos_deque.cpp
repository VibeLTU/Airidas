...
double median(const deque<double>& arr) {
    deque<double> temp = arr;
...
void generuotiBalus(deque<double>& nd_rezultatai, double& eg) {
    random_device rd;
    mt19937 gen(rd());
...
void atspausdintiDuomenis(const deque<Duomenys>& A, bool iFaila) {
    if (iFaila) {
...
void rikiuotiPagalVarda(deque<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.vardas < b.vardas;
    });
}

void rikiuotiPagalPavarde(deque<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.pavarde < b.pavarde;
    });
}

void rikiuotiPagalGalutiniVidurki(deque<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.galutinis < b.galutinis;
    });
}

void rikiuotiPagalMediana(deque<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.mediana < b.mediana;
    });
}

void skaitytiIsFailo(deque<Duomenys>& A, const string& failoPavadinimas) {
...
void rikiuotiStudentus(deque<Duomenys>& geri_studentai, deque<Duomenys>& blogi_studentai) {
