...
struct duomenys {
  string vardas;
  string pavarde;
  int nd; // namu darbu rezultatai
  int eg; // egzamino rezultatai
...
int main() {
  ...
  for (int i = 0; i < m ; i++){//accumulate panaudoti, kad butu greiciau veliau skaiciuoti vidurki
    A[i].ndvid = 0;
    for(int j=0; j<n; j++){
      A[i].ndvid = A[i].ndvid + A[j].nd;
    }
