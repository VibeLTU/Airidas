...
double median(list<double>& arr) {
    list<double> temp = arr;
    temp.sort();
    int size = temp.size();
    auto it = temp.begin();
    advance(it, size / 2);
    if (size % 2 == 0) {
        double first = *prev(it);
        double second = *it;
...
void generuotiBalus(list<double>& nd_rezultatai, double& eg) {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> nd_distribution(1, 10);
    uniform_int_distribution<> eg_distribution(1, 10);

    auto it = nd_rezultatai.begin();
    for (; it != nd_rezultatai.end(); ++it) {
        *it = nd_distribution(gen);
    }
    eg = eg_distribution(gen);
}

void atspausdintiDuomenis(const list<Duomenys>& A, bool iFaila) {
    if (iFaila) {
...
void rikiuotiPagalVarda(list<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.vardas < b.vardas;
    });
}

void rikiuotiPagalPavarde(list<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.pavarde < b.pavarde;
    });
}

void rikiuotiPagalGalutiniVidurki(list<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.galutinis < b.galutinis;
    });
}

void rikiuotiPagalMediana(list<Duomenys>& A) {
    sort(A.begin(), A.end(), [](const Duomenys& a, const Duomenys& b) {
        return a.mediana < b.mediana;
    });
}

void skaitytiIsFailo(list<Duomenys>& A, const string& failoPavadinimas) {
...
void rikiuotiStudentus(list<Duomenys>& geri_studentai, list<Duomenys>& blogi_studentai) {
